...
| 连续 3 个完整有效帧 io_state=0x00100000 | B0 低，C2 低 |
| 其他有效 io_state（包括0x00000000） | B0 保持，C2 低 |
| 无效/坏帧（校验失败） | 输出不变，但清零关断计数 |
...
升级/首次启动会将 B0 设高、C2 设低；B0 仅在连续收到 3 个明确关机值 0x00100000
的有效帧后拉低。中间任何其他有效帧、无效/坏帧或 socket 断开都会重置计数
（宁可晚关断也不误关）。
请在允许切换电源控制信号时操作。
